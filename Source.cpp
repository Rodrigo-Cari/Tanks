...
{
    // Crear una ventana
    sf::RenderWindow window(sf::VideoMode::getDesktopMode(), "SFML Tank Game", sf::Style::Fullscreen);
    ...
    // Establecer el origen del sprite al centro del tanque para facilitar la rotación
    sprite.setOrigin(134 / 2, 92 / 2);
    sprite.setPosition(window.getSize().x/2, window.getSize().y/2); // Centrando el tanque en la ventana

    // Crear un rectángulo que represente las paredes del laberinto
    sf::Vector2u windowSize = window.getSize();
    sf::RectangleShape maze(sf::Vector2f(windowSize.x * 0.9f, windowSize.y * 0.9f));
    ...
    while (window.isOpen())
    {
        ...
        timeSinceLastShot += deltaTime; // Actualizar el tiempo desde el último disparo
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
        {
            ...
            movement.x += cos(sprite.getRotation() * 3.14159265 / 180) * moveSpeed * deltaTime;
            movement.y += sin(sprite.getRotation() * 3.14159265 / 180) * moveSpeed * deltaTime;
        }
        ...
        if (mazeBounds.contains(nextPos.left, nextPos.top) &&
            ...
            mazeBounds.contains(nextPos.left + nextPos.width, nextPos.top + nextPos.height))
        {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space) && timeSinceLastShot >= fireRate)
        {
            Bullet bullet;
            ...
            bullet.velocity = sf::Vector2f(cos(sprite.getRotation() * 3.14159265 / 180) * bulletSpeed,
        ...
        for (auto& bullet : bullets)
        {
            ...
            if (bulletBounds.left <= mazeBounds.left || bulletBounds.left + bulletBounds.width >= mazeBounds.left + mazeBounds.width)
            {
                bullet.velocity.x = -bullet.velocity.x;
                bullet.shape.move(bullet.velocity.x * deltaTime, 0); // Evitar que se quede pegada
            }
            if (bulletBounds.top <= mazeBounds.top || bulletBounds.top + bulletBounds.height >= mazeBounds.top + mazeBounds.height)
            {
                bullet.velocity.y = -bullet.velocity.y;
                bullet.shape.move(0, bullet.velocity.y * deltaTime); // Evitar que se quede pegada
            }
        }
